---
front_matter_title: 어떻게 상수로 선언한 오브젝트의 속성값이 바뀌는 것일까
author: Seungwoo Lee
date: 2022-01-31
...
# 어떻게 상수로 선언한 오브젝트의 속성값이 바뀌는 것일까
...
const 키워드를 사용하여 선언된 변수에 숫자나 문자열과 같은 원시 타입은 메모리상에 실제 값이 전달이 되는 것이고 객체나 배열인 경우에는 참조에 의한 전달로 주솟값이 전달되는 것이라서 할당된 주솟값을 변경할 수는 없지만 주소에 할당된 값의 속성이나 항목은 변경할 수 있는 것입니다.
